...
# Utils
# -----------------------------
def format_input_waveform(x):
    """
    Ensures waveform tensor is in (B, 1, T) format for Conv1D layers.
    
    Args:
        x (Tensor): Input waveform tensor. Can be (B, T) or (B, T, 1) or (B, C, T).
    
    Returns:
        Tensor: Formatted tensor of shape (B, 1, T)
    """
    if x.dim() == 2:
