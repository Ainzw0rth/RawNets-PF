...
def train_rawnet2_with_loaders(model, train_loader, val_loader=None, device="cuda", epochs=100, lr=0.0001, start_epoch=0, variation="combined"):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        save_model_rawnet2(model, optimizer, scaler, epoch, path=f"pretrained_weights/{variation}/RawNet2/rawnet2_{variation}-ep_{epoch+1}-bs_{train_loader.batch_size}-lr_{lr}.pth")
...
def test_rawnet2_with_loaders(model, test_loader, device="cuda"):
    model.eval()
    predictions = []
    targets = []
    probs = []
