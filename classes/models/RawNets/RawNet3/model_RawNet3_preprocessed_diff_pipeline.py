...
class RawNet3Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # Split input: first 64000 and last 24 attributes
        # x shape: (batch_size, 64024)
        x_waveform = x[:, :64000]  # (batch_size, 64000)
        x_patho = x[:, 64000:]  # (batch_size, 24)

        # Pass first 64000 through filter layers
        with torch.amp.autocast(device_type='cuda', enabled=False):
            x_waveform = self.preprocess(x_waveform)
            x_waveform = torch.abs(self.conv1(x_waveform))
            if self.log_sinc:
                x_waveform = torch.log(x_waveform + 1e-6)
            if self.norm_sinc == "mean":
                x_waveform = x_waveform - torch.mean(x_waveform, dim=-1, keepdim=True)
            elif self.norm_sinc == "mean_std":
                m = torch.mean(x_waveform, dim=-1, keepdim=True)
