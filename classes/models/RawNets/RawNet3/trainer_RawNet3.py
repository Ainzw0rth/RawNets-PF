...
def train_rawnet3_with_loaders(model, train_loader, val_loader=None, device="cuda", epochs=100, lr=0.0001, start_epoch=0, variation="combined"):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_loader:
            metrics = validate_rawnet3(model, val_loader, device)
        ...
        save_model_rawnet3(model, optimizer, scaler, epoch, path=f"pretrained_weights/RawNet3/{variation}/rawnet3_{variation}-ep_{epoch+1}-bs_{train_loader.batch_size}-lr_{lr}.pth")
...
def save_model_rawnet3(model, optimizer, scaler, epoch, path="pretrained_weights/rawnet3.pth"):
...
def load_model_rawnet3(model, optimizer=None, scaler=None, path=None, device="cuda"):
