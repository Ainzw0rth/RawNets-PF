...
def train_wavlm_nes2net_with_loaders(model, train_loader, val_loader=None, device="cuda", epochs=100, 
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for inputs, labels in pbar:
            ...
            with autocast(enabled=False, dtype=torch.float16, cache_enabled=True):
                ...
                # Handle binary output from model (needs to be converted to 2-class)
                if outputs.shape[1] == 1:
                    # Convert single output to 2-class logits
        ...
        if val_loader:
            metrics = validate_wavlm_nes2net(model, val_loader, device)
        ...
        save_model_wavlm_nes2net(
            model, optimizer, scaler, epoch, 
            path=f"pretrained_weights/{variation}/WavLM_Nes2Net/wavlm_nes2net_{variation}-ep_{epoch+1}-bs_{train_loader.batch_size}-lr_{lr}.pth"
        )
...
def validate_wavlm_nes2net(model, val_loader, device="cuda"):
    """
    ...
    with torch.no_grad():
        for inputs, labels in val_loader:
            ...
            # Handle binary output from model
            if outputs.shape[1] == 1:
                # Convert single output to probability
...
def test_wavlm_nes2net(model, test_loader, class_labels=None, device="cuda"):
    """
...
def test_wavlm_nes2net_with_loaders(model, test_loader, device="cuda"):
    """
...
def save_model_wavlm_nes2net(model, optimizer, scaler, epoch, path="pretrained_weights/wavlm_nes2net.pth"):
    """
...
def load_model_wavlm_nes2net(model, optimizer=None, scaler=None, path=None, device="cuda"):
    """
...
def produce_evaluation_file(model, dataset, device, save_path):
    ...
    with torch.no_grad():
        for batch_x, utt_id in data_loader:
            ...
            if batch_out.shape[1] == 1:
                batch_score = torch.sigmoid(batch_out.squeeze(1)).data.cpu().numpy().ravel()
            else:
