...
def train_xlsr_conformer_tcm_with_loaders(model, train_loader, val_loader=None, device="cuda", epochs=100, 
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_loader:
            metrics = validate_xlsr_conformer_tcm(model, val_loader, device)
        ...
        save_model_xlsr_conformer_tcm(
            ...
            path=f"pretrained_weights/{variation}/XLSR_Conformer_TCM/xlsr_conformer_tcm_{variation}-ep_{epoch+1}-bs_{train_loader.batch_size}-lr_{lr}.pth"
        )

    print("Training completed.")
    total_time = time.time() - total_start_time
    print(f"\nTotal training time for XLSR-Conformer with TCM: {total_time:.2f} seconds ({total_time / 60:.2f} minutes)")
...
def validate_xlsr_conformer_tcm(model, val_loader, device="cuda"):
    ...
    with torch.no_grad():
        for inputs, labels in val_loader:
            inputs, labels = inputs.to(device), labels.to(device)
            outputs, _ = model(inputs)
            probs = torch.softmax(outputs, dim=1)[:, 1]  # assuming class 1 is "bonafide"
            loss = criterion(outputs, labels)
...
def test_xlsr_conformer_tcm(model, test_loader, class_labels=None, device="cuda"):
    ...
    plt.title("Confusion Matrix - XLSR-Conformer with TCM")
    plt.show()
...
def save_model_xlsr_conformer_tcm(model, optimizer, scaler, epoch, path="pretrained_weights/xlsr_conformer_tcm.pth"):
    """
    Save XLSR-Conformer with TCM model checkpoint
    ...
    print(f"Model XLSR-Conformer with TCM saved to {path}")


def load_model_xlsr_conformer_tcm(model, optimizer=None, scaler=None, path=None, device="cuda"):
    """
    Load XLSR-Conformer with TCM model checkpoint
