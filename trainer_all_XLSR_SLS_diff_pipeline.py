...
if __name__ == "__main__":
    ...
    log_filename = f"logs/train/train_log_xlsr_sls_diff_pipeline_{timestamp}.txt"
    ...
    try:
        for batch_size in batch_sizes:
            ...
            for lr_idx, learning_rate in enumerate(learning_rates):
                ...
                # -----------------------------
                # XLSR-SLS Diff Pipeline (with Pathological Features)
                # -----------------------------
                ...
                model = XLSRSLSDiffPipeline(model_config, device).to(device)
                ...
                # Train XLSR-SLS Diff Pipeline
                print("\n=== Training XLSR-SLS Diff Pipeline ===")
                train_xlsr_sls_with_loaders(
                    model, 
                    train_loader, 
                    val_loader, 
                ...
                # Test XLSR-SLS Diff Pipeline
                print("\n--- Testing XLSR-SLS Diff Pipeline ---")
                predictions, targets, metrics = test_xlsr_sls(model, test_loader, device=device)
